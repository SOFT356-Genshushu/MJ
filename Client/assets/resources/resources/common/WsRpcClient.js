...
WsRpcClient.prototype.startConnectUntilConnected = function (url) {//Start connecting until the connection is successful, only effective when the first connection is made
    var self = this;
    this.url = url;
    this.client = new WebSocket(this.url);
    this.client.binaryType = "arraybuffer";
    ...
    this.client.onclose = function (evt) {
        self.stopCbTimeOut();
        if (self.isReady == true) {//If you just started connecting
            self.events.emit("onClose", self);
            self.isReady = false;
        }
        setTimeout(function () {
